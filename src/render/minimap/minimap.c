...
static void	put_pixel_to_img(t_game *game, int *img_data, int line_len, int x,
		int y, int color)
{
		...
		int y)
{
	...
	while (i < 8)
	{
		put_pixel_to_img(game, img_data, line_len, x + (int)(game->player.dir_x
				* i), y + (int)(game->player.dir_y * i), 0x00FF00);
		i++;
	}
}
